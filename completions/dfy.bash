...
_dfy_complete() {
  ...
  local -a subcommands=(link unlink adopt list info create delete init config status doctor update uninstall help)
  local -a global_flags=(--help -h --version -V --no-color --dry-run --profile -p --yes -y --dir -d --lang -l)

  local i
  for ((i = 1; i < COMP_CWORD; i++)); do
    case "${COMP_WORDS[i]}" in
      link | unlink | adopt | list | info | create | delete | init | config | status | doctor | update | uninstall | help)
        subcmd="${COMP_WORDS[i]}"
        break
        ;;
    esac
