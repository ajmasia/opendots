...
# Locale directory is resolved from DOTS_LOCALES (if set) or from this file's
# real path so that bin/dots can be symlinked anywhere.
i18n::load() {
  local lang="${1:-en}"
  local locales_dir
  if [[ -n "${DOTS_LOCALES:-}" ]]; then
    locales_dir="$DOTS_LOCALES"
  else
